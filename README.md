👋 Hi, I'm Anand  

Aspiring Data Analyst | Power BI • Excel • Data Visualization  

---

## 👨‍💻 About Me

📊 I am an aspiring Data Analyst with a strong interest in turning raw data into meaningful insights.  
📈 I enjoy working on real-world datasets, performing data cleaning, analysis, and building interactive dashboards.  
🧠 Currently focusing on improving my skills in Power BI, Excel, and data storytelling.  
🎯 My goal is to become a job-ready Data Analyst and work on impactful data-driven projects.

---

## 🛠 Skills & Tools

- 📊 Power BI (Data Cleaning, DAX Basics, Dashboards)  
- 📈 Microsoft Excel (Data Analysis, Pivot Tables, Regression)  
- 🧹 Data Cleaning & Data Preparation  
- 📉 Data Visualization & Business Insights  
- 🗂 CSV / Structured Datasets  

---

## 📂 Featured Projects

🔹 **Fast Food Nutrition Analysis**  
- Cleaned raw nutrition data using Excel & Power BI  
- Built an interactive dashboard to analyze calories, fat, sodium, and protein  
- Identified healthy vs unhealthy menu patterns  

🔹 **Business Profit Regression Analysis**  
- Performed linear regression analysis using Excel  
- Analyzed impact of R&D, Marketing, and Administration spend on profit  
- Created Power BI dashboard for visualization  

👉 Check my repositories to explore more projects.

---

## 🌱 Currently Learning
- Advanced Power BI & DAX  
- SQL for Data Analysis  
- Real-world business analytics use cases  

---

## 🌐 Connect With Me

<!-- 🔴 CHANGE USERNAME & EMAIL BELOW 🔴 -->

<a href="https://www.instagram.com/anand_.0013/" target="_blank">
  <img src="https://img.icons8.com/fluency/48/instagram-new.png" alt="Instagram"/>
</a>
&nbsp;&nbsp;
<a href="mailtoaprajapati2074@gmail.com">
  <img src="https://img.icons8.com/fluency/48/gmail-new.png" alt="Email"/>
</a>
&nbsp;&nbsp;
<a href="https://www.linkedin.com/in/anandprajapati13/" target="_blank">
  <img src="https://img.icons8.com/fluency/48/linkedin.png" alt="LinkedIn"/>
</a>

---

📍 India  
💼 Open to learning, collaboration, and entry-level data analyst opportunities  

---

⭐ *This profile showcases my journey and projects in Data Analytics.*
